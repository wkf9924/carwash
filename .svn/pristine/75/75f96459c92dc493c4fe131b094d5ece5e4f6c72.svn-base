    ...
    [geocoder reverseGeocodeLocation:marsLoction completionHandler:^(NSArray *placemarks,NSError *error)
     {
         if (placemarks.count > 0) {
             ...
             [[NSUserDefaults standardUserDefaults] setObject: _lastCity    forKey:@"LASTCITY"];
             [[NSNotificationCenter defaultCenter] postNotificationName:CITY_DISPLAY object:_lastCity];
             _lastAddress = placemark.name;
             NSLog(@"______%@",_lastAddress);
             NSLog(@"详细地址：%@%@%@%@",placemark.locality,placemark.subLocality,placemark.thoroughfare,placemark.subThoroughfare);
             NSString *address = [NSString stringWithFormat:@"%@%@%@%@",placemark.locality,placemark.subLocality,placemark.thoroughfare,placemark.name];
