...
#import "BZSettingLabelItem.h"
#import "BZDefine.h"

@interface BZSettingCell()
/**
 *  箭头
 */
...
- (void)setupBg
{
    ...
    selectedBg.backgroundColor = BZColorRGB(237, 233, 218);
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    BZSettingCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[BZSettingCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)setItem:(BZSettingItem *)item
{
    _item = item;
...
{
    if ([self.item isKindOfClass:[BZSettingArrowItem class]]) { // 箭头
        self.accessoryView = self.arrowView;
        self.selectionStyle = UITableViewCellSelectionStyleDefault;
    } else if ([self.item isKindOfClass:[BZSettingSwitchItem class]]) { // 开关
    ...
    } else if ([self.item isKindOfClass:[BZSettingLabelItem class]]) { // 标签
