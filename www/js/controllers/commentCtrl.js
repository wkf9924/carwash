define(function () {
    'use strict';
    	function ctrl($scope,CarWashRequest,getGoodList,getTokenR,$ionicHistory,$state,$log,$stateParams,confirmPopu,$timeout,customeLoading) {
          	/*没有定义数组长度为1 接口获取我的订单*/
        	...
        	function creatSentParas(){
        		var attrArr=["prodcut_id","order_prodcut_id"];
        		$scope.sendParas={};
    ...
    ctrl.$inject = ['$scope',"CarWashRequest",'getGoodList',"getTokenR","$ionicHistory","$state","$log","$stateParams","confirmPopu","$timeout","customeLoading"];//顺序
    return ctrl;
});
